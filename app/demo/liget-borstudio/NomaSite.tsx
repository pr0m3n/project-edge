...
export function NomaSite() {
  ...
  const [specialist, setSpecialist] = useState("Bori");
  const [guestName, setGuestName] = useState("");
  const [guestEmail, setGuestEmail] = useState("");
  const [bookingOpen, setBookingOpen] = useState(false);
  const finish = () => notice(`A ${treatments[treatment].name} · ${slot} · ${specialist} időpontot nem foglaltuk le — ez egy interaktív mintaprojekt.`);
  ...
  return (
    <div className="noma-root" id="top">
      ...
      {bookingOpen && <div className="noma-modal" role="dialog" aria-modal="true" aria-label="Időpontfoglalás"><button className="noma-close" onClick={()=>setBookingOpen(false)} aria-label="Bezárás" type="button">×</button><div className="noma-booking-head"><span>01 / 03</span><h2>Válassz kezelést</h2></div><div className="noma-treatment-pills">{treatments.map((item,index)=><button className={treatment===index?"active":""} onClick={()=>setTreatment(index)} key={item.name} type="button"><strong>{item.name}</strong><span>{item.time} · {item.price}</span></button>)}</div><div className="noma-booking-head"><span>02 / 03</span><h2>Nap és időpont</h2></div><div className="noma-days">{["H 10","K 11","SZE 12","CS 13"].map((item,index)=><button className={day===index?"active":""} onClick={()=>setDay(index)} key={item} type="button">{item}</button>)}</div><div className="noma-slots">{slots.map(item=><button className={slot===item?"active":""} onClick={()=>setSlot(item)} key={item} type="button">{item}</button>)}</div><div className="noma-booking-head"><span>03 / 03</span><h2>Szakember és elérhetőség</h2></div><div className="noma-specialists">{["Bori","Kata","Bárki megfelelő"].map(item=><button className={specialist===item?"active":""} onClick={()=>setSpecialist(item)} key={item} type="button">{item}</button>)}</div><div className="noma-guest-fields"><input aria-label="Név" onChange={event=>setGuestName(event.target.value)} placeholder="Neved" value={guestName} /><input aria-label="E-mail-cím" onChange={event=>setGuestEmail(event.target.value)} placeholder="E-mail-címed" type="email" value={guestEmail} /></div><div className="noma-summary"><div><span>Kiválasztva</span><strong>{treatments[treatment].name} · {slot} · {specialist}</strong></div><button disabled={!guestName || !guestEmail} onClick={finish} type="button">Foglalás megerősítése →</button></div></div>}
