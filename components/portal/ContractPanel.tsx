...
import { BUYOUT_CREDIT_MAX_SHARE, BUYOUT_CREDIT_RATE, PRICE_TAX_NOTE, formatHuf, subscriptionPlan } from "@/lib/subscriptions";
...
function sections(project: Project) {
  ...
  return [
    ...
    ["3", "Díj és fizetés", managed
      ? `Díj: ${price}, a választott fizetési ütemezés szerint előre. Induló, belépési vagy beállítási díj NINCS. A díj először akkor esedékes, amikor az Ügyfél az elkészült weboldalt az Ügyfélkapun jóváhagyta — tehát az Ügyfél a kész, általa elfogadott weboldalért fizet, az élesítés előtt. A további díjakat a fizetési szolgáltató minden szolgáltatási időszak elején automatikusan terheli, amíg az Ügyfél fel nem mondja. Banki átutalás féléves és éves ütemezésnél választható. A kártyaadatot a Szolgáltató nem tárolja. ${PRICE_TAX_NOTE}`
    ...
    ["4", "Határidő és együttműködés", `${project.offer_timeline || (managed ? "A kivitelezés a jelen szerződés elfogadása és a szükséges anyagok hiánytalan átadása után indul — fizetés nélkül." : "Az elfogadott ajánlat szerint.")} Az ügyfél késedelmes anyagátadása vagy visszajelzése a határidőt arányosan kitolja.`],
    ...
    ["7", managed ? "Időtartam, szüneteltetés és felmondás" : "Átadás és hibajavítás", managed
      ? `A szerződés határozatlan időre jön létre, hűségidő nélkül. Bármely hónapban felmondható; a felmondás a folyó, kifizetett időszak végén hatályos. Szüneteltetés a mindenkori parkolási díj mellett kérhető. A weboldal külön vételáron bármikor megvásárolható: minden rendezett előfizetési hónap után a havidíj ${Math.round(BUYOUT_CREDIT_RATE * 100)}%-a beszámít a vételárba, mindaddig, amíg a beszámítás el nem éri a vételár ${Math.round(BUYOUT_CREDIT_MAX_SHARE * 100)}%-át. A beszámítás nem ruházható át, készpénzre nem váltható, és az előfizetés megszűnésével elvész.`
    ...
    ["8", "Jóváhagyás és az élesítés feltétele", managed
      ? `A Szolgáltató az elkészült weboldalt előnézeti (staging) címen mutatja be. Az Ügyfél az Ügyfélkapun nyilatkozik a jóváhagyásról; a jóváhagyás időpontját a rendszer rögzíti. A jóváhagyás azt jelenti, hogy a weboldal az elfogadott ajánlatnak és briefnek megfelelően elkészült. A jóváhagyást követően esedékes az első díj. A weboldal az Ügyfél saját domainjén akkor kerül élesítésre, amikor az első díj a Szolgáltatóhoz megérkezett. Az élesítésig a weboldal a Szolgáltató infrastruktúráján marad, és azon fennálló minden rendelkezési jog a Szolgáltatót illeti; az Ügyfél a weboldalt vagy annak részeit nem jogosult használni, másolni vagy harmadik félnek átadni. Nemfizetés esetén a Szolgáltató jogosult a megrendelést elállással megszüntetni, és a weboldal nem kerül átadásra.`
      : `Az Ügyfél az Ügyfélkapun nyilatkozik az elkészült munka jóváhagyásáról; a jóváhagyás időpontját a rendszer rögzíti. Hozzáférés- és fájlátadás kizárólag a teljes díj rendezése után történik.`],
    ["9", "Fogyasztói nyilatkozat", `Fogyasztó ügyfél szolgáltatási szerződésnél 14 napon belül indokolás nélkül felmondhat. Ha külön kéri a teljesítés korábbi megkezdését, felmondáskor a már teljesített szolgáltatás arányos díját köteles megfizetni. A teljes szolgáltatás befejezésével a jog csak előzetes kifejezett kérés és tudomásulvétel mellett szűnik meg.`],
    ["10", "Felelősség, panasz és záró szabályok", `A felek együttműködnek és a másikat érintő körülményről késedelem nélkül tájékoztatnak. A Szolgáltató nem felel az ügyfél jogsértő tartalmáért vagy ellenőrzési körén kívüli szolgáltatáskiesésért, a kötelező jogszabályi felelősség korlátozása nélkül. Panasz: ${PROVIDER.email}. Az egyedi szerződés, az elfogadott brief és az ÁSZF együtt alkotja a megállapodást; eltérésnél az egyedi szerződés az elsődleges. Irányadó jog: magyar jog.`]
  ] as const;
}
...
export function contractPlainText(project: Project) {
  ...
  return [title, `Verzió: 2026-09-22`, `Szolgáltató: ${providerContractParty()}`, `Ügyfél: ${project.company || project.contact_name || "Megrendelő"} (${project.contact_email})`, `Projekt: ${project.title}`, ...sections(project).map(([, sectionTitle, copy]) => `${sectionTitle}\n${copy}`), `Az ÁSZF elérhetősége: https://projectedge.hu/aszf`].join("\n\n");
}
...
export function ContractPanel({ project, contractChecked, onContractCheckedChange, performanceConsent, onPerformanceConsentChange, onAccept }: Props) {
  ...
  return <article className="contract-document">
    <header className="contract-cover"><div><span className="contract-kicker">PROJECTEDGE · SZERZŐDÉS</span><h3>{managed ? "Menedzselt weboldal-szolgáltatás" : "Egyedi vállalkozási szerződés"}</h3><p>{managed ? "Most nem fizetsz semmit: a díj akkor esedékes, amikor a kész weboldalt jóváhagytad. Elfogadás előtt a teljes dokumentum nyomtatható." : "Átlátható feltételek, egy helyen. Elfogadás előtt a teljes dokumentum nyomtatható."}</p></div><div className="contract-seal"><span>{managed ? plan.name : "EGYEDI"}</span><strong>{managed ? formatHuf(project.monthly_price ?? plan.price) : formatPrice(project.offer_price, project.offer_currency || "Ft")}</strong><small>{managed ? "havonta" : "vállalási díj"}</small></div></header>
    ...
    {/* Menedzselt konstrukcióban a szerződés elfogadásakor tényleg 0 Ft
        fizetendő: a díj a kész oldal jóváhagyása után esedékes. A kártya ezt
        mondja ki, nem egy általános „nincs belépési díj"-at. */}
    <div className="contract-highlights"><div><b>{managed ? "0 Ft" : formatPrice(project.deposit_amount, project.offer_currency || "Ft")}</b><span>{managed ? "most fizetendő — a díj a jóváhagyás után esedékes" : "foglaló"}</span></div><div><b>{managed ? "Nincs" : "30 nap"}</b><span>{managed ? "hűségidő" : "hibajavítás"}</span></div><div><b>{managed ? "Teljes" : "Vezetett"}</b><span>{managed ? "technikai kezelés" : "átadás"}</span></div></div>
